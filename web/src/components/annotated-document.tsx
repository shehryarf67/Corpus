...
/**
 * Decorative hero illustration for the auth screen: the page flips once on
 * the X-axis, then gets a passage highlighted, a connector drawn, and a
 * citation chip attached — the product's core loop, on a repeating 10s
 * timeline.
 *
 ...
 * finished state.
 *
 * The drift and the flip live on separate elements on purpose: two
 * animations can't both drive `transform` on one node — the later one
 * would simply win. `.doc-perspective` is static (just gives the flip
 * somewhere to have 3D depth); `.doc-float` drifts and tilts; `.doc-flip`
 * does the X-axis rotation.
 */

import Link from "next/link";
import { TopBar } from "@/components/top-bar";

export default function NotFound() {
  return (
    <div className="flex min-h-[100dvh] flex-col">
      <TopBar />

      <main className="flex flex-1 items-center justify-center px-6 py-16">
        <div className="w-full max-w-[520px] text-center">
          <div className="font-mono text-[10.5px] tracking-[0.14em] text-graphite-dim uppercase">
            404
          </div>

          <h1 className="mt-4 font-serif text-[32px] leading-[1.1] font-semibold tracking-[-0.02em]">
            Document not found.
          </h1>

          <p className="mx-auto mt-3 max-w-[42ch] font-serif text-[14.5px] leading-[1.66] text-graphite">
            This document may have been removed, or you may not have access to
            it.
          </p>

          <Link
            href="/documents"
            className="mt-7 inline-block rounded-[3px] bg-marker px-4 py-2.5 text-[13px] font-semibold text-[#171004] transition hover:brightness-[1.08] active:translate-y-px"
          >
            Back to documents
          </Link>
        </div>
      </main>
    </div>
  );
}